...
function initDemandFilters() {
  ...
  function updateHeaderText(header, selections, allItems) {
    ...
    if (selections.length === 0) {
      ...
      textSpan.textContent = selectedItem
        ? selectedItem.name
  ...
  function loadProducts() {
    ...
    fetch(url)
      ...
      .then((data) => {
        ...
        productCount.textContent = `${data.length} product${
          data.length !== 1 ? "s" : ""
        }`;
        ...
        productsGrid.innerHTML = data
          .map(
            (product) => `
                    <div class="product-list-item" data-product-id="${
                      product.id
                    }">
        ...
        document.querySelectorAll(".product-checkbox").forEach((checkbox) => {
          checkbox.addEventListener("change", function () {
            this.closest(".product-list-item").classList.toggle(
              "selected",
              this.checked
            );
          });
  ...
  const updatedDeptHeader = setupDropdownToggle(
    departmentHeader,
    departmentDropdown
  );
  const updatedCatHeader = setupDropdownToggle(
    categoryHeader,
    categoryDropdown
  );
      ...
      document.querySelectorAll(".dept-checkbox").forEach((checkbox) => {
        checkbox.addEventListener("change", function () {
          ...
          const allChecked = Array.from(
            document.querySelectorAll(".dept-checkbox")
          ).every((cb) => cb.checked);
  ...
  function handleDepartmentChange() {
    ...
    const departmentIds = selectedDepartments.join(",");
    fetch(
      `testing/product_handler.php?action=get_categories&department_ids=${departmentIds}`
    )
      .then((res) => res.json())
      .then((data) => {
        if (data.length === 0) {
  ...
  function searchProducts() {
    ...
    let url = `testing/product_handler.php?action=search_products&search=${encodeURIComponent(
      search
    )}`;
    ...
    fetch(url)
          ...
          .map(
            (p) => `
                    <div class="search-item" data-id="${p.id}">
                        <strong>${
                          p.product_code || p.new_product_code
      ...
      .catch((err) => {
        ...
        searchResults.innerHTML =
          '<div class="no-results">Error searching</div>';
        searchResults.style.display = "block";
      });
  }
